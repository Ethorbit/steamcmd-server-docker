...
build:
	...
	find $(files) -maxdepth 0 -type d -exec \
		...
		docker build $(args) -t $$dirname -t $(docker_user)/$$dirname:latest -t $(docker_user)/$$dirname:$(git_hash) {} &&\
		[ 1 = 0 ] && docker volume rm -f $$dirname' \;

build-one:
	docker build $(args) -t $(image) -t $(docker_user)/$(image):latest -t $(docker_user)/$(image):$(git_hash) ./servers/$(image)
